...
def main():
    ...
    for rank in ranks:
        for seed in seeds:
            file = dir + "dat_cp_enes_L16_g0.5000000000_D8_rank{}_seed{}".format(rank, seed)
...
#    plt.ylim(-0.01,0.15)
    ...
    for irank,rank in enumerate(ranks):
        for iseed,seed in enumerate(seeds):
            ...
            cnt += 1
    plt.text(-0.125,0.85,r"(a)",fontsize=36,ha="center",va="center",transform=plt.gca().transAxes)
    plt.legend(ncols=1,columnspacing=1,handletextpad=0,fontsize=24,loc="upper left")
    plt.savefig("fig_ene_errors_vs_rank.pdf",bbox_inches="tight")
    plt.close()
