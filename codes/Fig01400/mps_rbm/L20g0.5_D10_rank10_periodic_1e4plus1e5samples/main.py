...
## exact
def ground_state_energy_tfim(N, h, J=1.0): # periodic only
    lambda_ = h / J
...
@numba.jit(nopython=True)
def run_vmc(n,J,h,q,n_th,n_samples,n_iter,n_ave,lr,diag_shift,b,W,seed):
...
#        for j in range(n_samples): # samples for thermalization
        for j in range(n_th): # samples for thermalization
            for _ in range(n):
...
#
    ...
    n_ave = 100
    lr = 0.01
    bs, Ws, E_means = run_vmc(n,J,h,q,n_th,n_samples,n_iter,n_ave,lr,diag_shift,b0s,W0s,seed)
...
#
    ...
    n_ave = 0
    lr = 0.00
    _, _, E_means = run_vmc(n,J,h,q,n_th,n_samples,n_iter,n_ave,lr,diag_shift,bs,Ws,seed+1)
...
#
    ...
    np.savetxt("dat_vmc_enes_ave_err"+string,
        np.array([
            L,g,D,rank,seed,e_exact,e_dmrg,
#            L,g,D,rank,seed,e_exact,e_dmrg,e_cp,
            Eave,Eerr,
            np.abs((Eave-e_exact)/e_exact),np.abs((Eerr)/e_exact)
        ]).reshape(1,-1),
        header="L g D rank seed exact(priodic) dmrg(open) vmc(periodic) vmc_err (vmc-exact)/exact vmc_err/exact")
#        header="L g D rank seed exact(priodic) dmrg(open) cpd(open) vmc(periodic) vmc_err (vmc-exact)/exact vmc_err/exact")
